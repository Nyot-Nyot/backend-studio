...
> **Catatan eksperimen:** Terdapat fitur _optional_ yang menggunakan model AI (Google Gemini) untuk _auto-generate_ response atau endpoint configs. Fitur ini bersifat **eksperimental** dan membutuhkan API key yang disediakan pengguna (tidak ada kunci default). Hasil generator AI dapat bervariasi; pastikan untuk meninjau dan memverifikasi output sebelum digunakan di workspace atau saat mengekspor.

-   **Feature flags:** Experimental features are configurable via localStorage (`feature_gemini`, `feature_sw`, `feature_logviewer`, `feature_proxy`, `feature_export`) or environment variables (`VITE_ENABLE_*`). The Traffic Monitor (live logs) and Service Worker interception are enabled by default to restore live traffic observation; other experimental features (AI Gemini, Proxy, Export Server) remain behind feature flags and are disabled unless explicitly enabled.
