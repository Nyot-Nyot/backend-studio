...
> **Feature flags:** Experimental features (AI Gemini, Service Worker interception, Traffic Monitor, Proxy, Export Server) sekarang dijaga di balik feature flags (local storage atau env vars). Secara default fitur-fitur ini tidak aktif agar produk lebih dekat dengan parity MockAPI; aktifkan via localStorage (`feature_gemini`, `feature_sw`, `feature_logviewer`, `feature_proxy`, `feature_export`) atau environment variables prefiks `VITE_ENABLE_*`.
