...
app.get("/openrouter/health", (req, res) => {
	const keyPresent = !!OPENROUTER_API_KEY;
...
async function callOpenRouter(messages, model = "deepseek/deepseek-r1-0528:free", apiKey) {
	const keyToUse = apiKey || OPENROUTER_API_KEY;
...
app.post("/openrouter/generate-mock", async (req, res) => {
	...
	const key =
		OPENROUTER_API_KEY || (process.env.DEV_ALLOW_CLIENT_KEY === "1" && keyFromHeader ? keyFromHeader : null);
