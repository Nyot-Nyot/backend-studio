...
function App() {
  ...
  const [userApiKey, setUserApiKey] = useState(
    () => localStorage.getItem("api_sim_user_gemini_key") || ""
  );
  ...
  useEffect(() => {
    ...
    const loadedMocks = JSON.parse(
      localStorage.getItem(STORAGE_KEY_MOCKS) || "[]"
    );
    const loadedEnvVars = JSON.parse(
      localStorage.getItem(STORAGE_KEY_ENV_VARS) || "[]"
    );
      ...
      try {
        ...
        if (navigator.storage && navigator.storage.persist) {
          ...
          if (!isPersisted) {
            ...
            if (granted) {
              addToast(
                "Storage persistence granted. Your data is less likely to be cleared.",
                "success"
              );
  ...
  const handleSaveMock = (mock: MockEndpoint) => {
    setMocks((prev) => {
      ...
      if (exists) {
        return prev.map((m) =>
          m.id === mock.id ? { ...mock, projectId: activeProjectId } : m
        );
      }
      return [
        ...prev,
  ...
  const handleExportData = () => {
    ...
    link.href = url;
    link.download = `backend-studio-backup-${new Date()
      .toISOString()
      .slice(0, 10)}.json`;
  ...
  const handleImportData = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (!Array.isArray(data.projects) || !Array.isArray(data.mocks))
        ...
        if (
          ...
          if (data.projects.length > 0) {
            const currentActiveExists = data.projects.find(
  ...
  const handleExportOpenApi = () => {
    ...
    const link = document.createElement("a");
    link.href = url;
    link.download = `openapi-${currentProject.name
      .toLowerCase()
      .replace(/\s+/g, "-")}.json`;
    document.body.appendChild(link);
    link.click();
  ...
  const generatePackageJson = () => {
    const currentProjectName =
      projects.find((p) => p.id === activeProjectId)?.name || "backend-api";
    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden font-sans text-slate-900">
      <Sidebar
        ...
        onTriggerCommandPalette={() =>
          addToast("Command Palette coming soon", "info")
        }
      ...
      <main className="flex-1 overflow-auto relative bg-[#f8fafc]">
        {view === "dashboard" && (
          <Dashboard
            ...
            onBulkDelete={(ids) =>
              setMocks((prev) => prev.filter((m) => !ids.includes(m.id)))
            }
            onToggle={(id) =>
              setMocks((prev) =>
                prev.map((m) =>
          ...
          <div className="p-10 max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
            ...
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
              ...
              <p className="text-slate-500 text-sm mb-6 max-w-2xl leading-relaxed">
                Define global variables (like <code>base_url</code> or{" "}
                <code>api_token</code>) that can be reused in your endpoint
                responses using the syntax <code>{`{{variable_name}}`}</code>.
              </p>
                  ...
                  >
                    <div className="col-span-4 relative">
                      ...
                      <input
                        ...
                        value={v.key}
                        onChange={(e) =>
                          handleUpdateEnvVar(v.id, "key", e.target.value)
                        }
                    ...
                    <div className="col-span-7">
                      <input
                        ...
                        value={v.value}
                        onChange={(e) =>
                          handleUpdateEnvVar(v.id, "value", e.target.value)
                        }
                        placeholder="Value"
            ...
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
              ...
              <p className="text-slate-500 text-sm mb-6 max-w-2xl leading-relaxed">
                Enter your Google Gemini API Key to enable Magic Create and
                Auto-Generate features. The key is stored securely in your
            ...
            <div className="bg-slate-900 p-8 rounded-2xl shadow-xl border border-slate-700 text-white relative overflow-hidden group">
              <div className="relative z-10">
                ...
                <p className="text-slate-400 text-sm mb-6 max-w-2xl leading-relaxed">
                  Export your endpoints as a standalone Node.js server. Useful
                  for local development or deploying to cloud providers.
                </p>
            ...
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
              ...
              <p className="text-slate-500 text-sm mb-8 max-w-2xl leading-relaxed">
                Export your entire workspace including all projects and route
                configurations.
              </p>
              ...
              <div className="mt-6 p-4 bg-amber-50 text-amber-800 text-xs rounded-xl flex items-start border border-amber-100">
                ...
                <span className="leading-5">
                  <strong>Warning:</strong> Importing data will completely
              ...
              <div className="flex items-start gap-4 mb-8 bg-slate-800/50 p-4 rounded-xl border border-slate-700">
                ...
                <div>
                  ...
                  <p className="text-slate-400 text-sm leading-relaxed">
                    Since Backend Studio runs entirely in your browser, external
                    tools cannot access it directly. To deploy to the cloud (or
                    run locally), you must export the generated{" "}
                    <code>server.js</code> file.
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div>
                    ...
                    <div className="p-4 text-slate-300 space-y-2">
                      <div className="flex gap-2 opacity-50">
                        <span className="select-none">
                          # 1. Create folder & download files
                        </span>
                      </div>
                  ...
                  <p className="text-slate-500 text-xs mb-4">
                    Download both files below to start a real local server on
                    port 3000.
                  </p>
                  <div className="flex flex-col gap-2">
                    ...
                    >
                      <span className="flex items-center gap-2">
                        <Package className="w-4 h-4 text-orange-400" /> Download
                        package.json
                      </span>
                      <Download className="w-4 h-4 opacity-50 group-hover:opacity-100" />
                    </button>
                  ...
                  <div className="space-y-3">
                    <div className="p-3 rounded-lg border border-slate-700 bg-slate-800/30">
                      ...
                      <p className="text-slate-400 text-xs">
                        Download both <code>server.js</code> and{" "}
                        <code>package.json</code> from the left panel.
                      </p>
                    </div>
                    <div className="p-3 rounded-lg border border-slate-700 bg-slate-800/30">
                      <h5 className="text-slate-200 font-bold text-xs mb-1">
                    ...
                    <div className="p-3 rounded-lg border border-slate-700 bg-slate-800/30">
                      ...
                      <p className="text-slate-400 text-xs">
                        Link your repo to{" "}
                        <strong>Render, Railway, or Vercel</strong>. They will
