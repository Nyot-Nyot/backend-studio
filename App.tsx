...
import { MockEndpoint, Project, ViewState, LogEntry, TestConsoleState, HttpMethod, EnvironmentVariable } from './types';
...
import { Globe, Plus, Trash2, Key, ShieldCheck, Eye, EyeOff, Server, Rocket, FileText, FileCode, Download, Upload, AlertTriangle, RefreshCw, Copy, Package, Info, Terminal, Cloud, X } from 'lucide-react';
...
function App() {
  ...
  const handleMagicCreate = async () => {
    const prompt = window.prompt("Describe the endpoint you want to create (e.g. 'A GET users list with pagination')");
    if (!prompt) return;
  ...
  const handleExportData = () => {
    ...
    link.href = url; link.download = `backend-studio-backup-${new Date().toISOString().slice(0, 10)}.json`;
  ...
  const handleImportData = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (confirm(`Replace current workspace with ${data.projects.length} projects and ${data.mocks.length} routes?`)) {
      ...
      } catch (error) { addToast('Failed to import: ' + (error as Error).message, 'error'); }
    };
  ...
  const handleExportOpenApi = () => {
    ...
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url; link.download = `openapi-${currentProject.name.toLowerCase().replace(/\s+/g, '-')}.json`;
    document.body.appendChild(link); link.click(); document.body.removeChild(link); URL.revokeObjectURL(url);
...
`;
  ...
  const handleGenerateServer = () => {
    ...
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url; link.download = 'server.js'; document.body.appendChild(link); link.click(); document.body.removeChild(link); URL.revokeObjectURL(url);
  ...
  const handleDownloadPackageJson = () => {
      ...
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url; link.download = 'package.json'; document.body.appendChild(link); link.click(); document.body.removeChild(link); URL.revokeObjectURL(url);
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden font-sans text-slate-900">
      <Sidebar 
        ...
        onNewMock={() => { setEditingMock(null); setView('editor'); }}
      ...
      <main className="flex-1 overflow-auto relative bg-[#f8fafc]">
        {view === 'dashboard' && (
          <Dashboard 
            mocks={projectMocks}
            onEdit={(mock) => { setEditingMock(mock); setView('editor'); }}
            onDelete={handleDeleteMock}
            onBulkDelete={(ids) => setMocks(prev => prev.filter(m => !ids.includes(m.id)))}
            onToggle={(id) => setMocks(prev => prev.map(m => m.id === id ? { ...m, isActive: !m.isActive } : m))}
            onDuplicate={(mock) => {
               const newMock = { ...mock, id: crypto.randomUUID(), name: `${mock.name} (Copy)`, path: `${mock.path}-copy` };
           ...
           <div className="p-10 max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
             ...
             <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
                ...
                <p className="text-slate-500 text-sm mb-6 max-w-2xl leading-relaxed">
                  Define global variables (like <code>base_url</code> or <code>api_token</code>) that can be reused in your endpoint responses using the syntax <code>{`{{variable_name}}`}</code>.
                </p>
             ...
             <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
                ...
                <div className="max-w-xl space-y-4">
                    ...
                    <div className="flex items-center justify-between">
                         <a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noreferrer" className="text-xs font-medium text-violet-600 hover:text-violet-700 hover:underline">
                            Get a free API Key
                         </a>
                         <button 
             ...
             <div className="bg-slate-900 p-8 rounded-2xl shadow-xl border border-slate-700 text-white relative overflow-hidden group">
                <div className="relative z-10">
                  ...
                  <p className="text-slate-400 text-sm mb-6 max-w-2xl leading-relaxed">
                    Export your endpoints as a standalone Node.js server. Useful for local development or deploying to cloud providers.
                  </p>
             ...
             <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
                ...
                <div className="mt-6 p-4 bg-amber-50 text-amber-800 text-xs rounded-xl flex items-start border border-amber-100">
                   ...
                   <span className="leading-5"><strong>Warning:</strong> Importing data will completely replace your current workspace.</span>
                </div>
             </div>
             ...
             <div className="bg-[#0f172a] rounded-2xl shadow-2xl w-full max-w-3xl border border-slate-700 flex flex-col overflow-hidden max-h-[90vh]">
                <div className="p-6 border-b border-slate-800 flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-slate-400 text-sm mt-1">Export your endpoints as a standalone Node.js application.</p>
                    </div>
                    ...
                    <div className="flex items-start gap-4 mb-8 bg-slate-800/50 p-4 rounded-xl border border-slate-700">
                        ...
                        <div>
                            ...
                            <p className="text-slate-400 text-sm leading-relaxed">
                                Since Backend Studio runs entirely in your browser, external tools cannot access it directly. 
                                To deploy to the cloud (or run locally), you must export the generated <code>server.js</code> file.
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                         ...
                         <div>
                            ...
                            <div className="flex flex-col gap-2">
                                ...
                                >
                                    <span className="flex items-center gap-2"><FileCode className="w-4 h-4 text-emerald-500" /> Download server.js</span>
                                ...
                                <button
                                    ...
                                    className="px-4 py-2.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-white font-medium transition-all flex items-center justify-between group border border-slate-700"
                                >
                                    <span className="flex items-center gap-2"><Package className="w-4 h-4 text-orange-400" /> Download package.json</span>
                                    <Download className="w-4 h-4 opacity-50 group-hover:opacity-100" />
                                </button>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div className="space-y-3">
                                <div className="p-3 rounded-lg border border-slate-700 bg-slate-800/30">
                                    ...
                                    <p className="text-slate-400 text-xs">Download both <code>server.js</code> and <code>package.json</code> from the left panel.</p>
                                </div>
                                <div className="p-3 rounded-lg border border-slate-700 bg-slate-800/30">
                                    <h5 className="text-slate-200 font-bold text-xs mb-1">2. Push to Git</h5>
                                    <p className="text-slate-400 text-xs">Commit these files to a GitHub/GitLab repository.</p>
                                </div>
                                <div className="p-3 rounded-lg border border-slate-700 bg-slate-800/30">
                                    <h5 className="text-slate-200 font-bold text-xs mb-1">3. Connect Provider</h5>
                                    <p className="text-slate-400 text-xs">Link your repo to <strong>Render, Railway, or Vercel</strong>. They will auto-detect the Node.js app.</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="p-6 bg-slate-900 border-t border-slate-800 flex justify-end space-x-3">
                    ...
                    <button onClick={() => setIsDeployModalOpen(false)} className="px-5 py-2.5 rounded-xl bg-white text-slate-900 font-bold hover:bg-slate-200 transition-colors">
                        Done
                    </button>
                </div>
             </div>
