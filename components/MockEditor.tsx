...
import { Save, ArrowLeft, Wand2, AlertCircle, CheckCircle, Settings, Database, Plus, X, Gauge, Copy, FileJson, Trash2, Code2, Table2, GripVertical, ChevronDown, ChevronRight, Sparkles, ToggleLeft, ToggleRight, Hash, Type, FolderOpen, AlignLeft, Minimize, Clipboard, Check, Search, ZoomIn, ZoomOut, ArrowDown, ArrowUp, AlertTriangle, Shield, Key } from 'lucide-react';
...
const FieldRow: React.FC<{ field: SchemaField; depth?: number; ctx: FieldRowContextProps }> = ({ field, depth = 0, ctx }) => {
                ...
                <div className="flex-1 grid grid-cols-12 gap-3 items-center">
                    ...
                    <div className="col-span-4 flex items-center">
                        <input 
                            ...
                            className={`w-full text-sm font-semibold text-slate-700 bg-slate-50 focus:bg-white border border-transparent focus:border-brand-500 rounded-md py-1.5 px-2.5 outline-none transition-colors ${field.key === '' ? 'border-red-200 bg-red-50' : ''}`}
                        />
                    </div>
                    ...
                    <div className="col-span-3">
                        <div className="relative">
                            ...
                            <div className="absolute left-2.5 top-1/2 -translate-y-1/2 pointer-events-none text-slate-400">
                                {field.type === 'object' ? <FolderOpen className="w-3.5 h-3.5" /> : 
                                 field.type === 'string' ? <Type className="w-3.5 h-3.5" /> :
                                 field.type === 'number' ? <Hash className="w-3.5 h-3.5" /> :
                                 field.type === 'boolean' ? <ToggleRight className="w-3.5 h-3.5" /> :
                    ...
                    <div className="col-span-5 relative">
                        {field.type === 'object' ? (
                            ...
                            <div className="flex items-center h-9">
                                <button 
                                    type="button"
                                    onClick={() => ctx.onUpdateField(field.id, { value: field.value === 'true' ? 'false' : 'true' })}
                                ...
                                >
                                    {field.value === 'true' ? <ToggleRight className="w-4 h-4" /> : <ToggleLeft className="w-4 h-4" />}
                                    <span>{field.value === 'true' ? 'True' : 'False'}</span>
                                </button>
                            </div>
                        ) : field.type === 'null' ? (
                             <div className="text-xs text-slate-400 italic py-2 px-2 bg-slate-50 rounded border border-slate-100">null</div>
                        ) : (
                            <div className="relative flex items-center">
                                <input 
                                    ...
                                    className={`w-full text-sm text-slate-600 bg-white border border-slate-200 rounded-md py-1.5 px-2.5 pr-8 font-mono focus:border-brand-500 focus:ring-1 focus:ring-brand-500 outline-none transition-colors ${field.value.includes('{{') ? 'text-brand-600 font-bold' : ''} ${field.type === 'array' ? 'bg-slate-100 text-slate-500' : ''}`}
                                />
                                {field.type === 'array' && <span className="absolute right-3 text-xs text-slate-400 pointer-events-none">JSON</span>}
                                {field.type === 'string' && (
                                    <button
                                        type="button"
                                        onClick={() => ctx.onSetActiveGeneratorDropdown(ctx.activeGeneratorDropdown === field.id ? null : field.id)}
                                        className={`absolute right-1.5 p-1 rounded hover:bg-slate-100 transition-colors ${field.value.includes('{{') ? 'text-brand-500' : 'text-slate-400'}`}
                                ...
                                {ctx.activeGeneratorDropdown === field.id && (
                                    <div className="absolute top-full right-0 mt-1 w-48 bg-white rounded-lg shadow-xl border border-slate-200 z-50 animate-in fade-in zoom-in-95 duration-200 overflow-hidden">
                                        <div className="bg-slate-50 px-3 py-2 border-b border-slate-100 flex justify-between items-center">
                                            <span className="text-[10px] font-bold text-slate-500 uppercase">Generators</span>
                                            <button type="button" onClick={() => ctx.onSetActiveGeneratorDropdown(null)}><X className="w-3 h-3 text-slate-400" /></button>
                                        </div>
                                        <div className="max-h-48 overflow-y-auto p-1">
                                            {MOCK_VARIABLES_HELP.map(v => (
                                                ...
                                                >
                                                    <span className="text-xs font-mono font-medium text-brand-600">{v.label}</span>
                                                    <span className="text-xs text-slate-400 group-hover/item:text-slate-500">{v.desc}</span>
                                                </button>
                                            ))}
                                        </div>
                                    </div>
...
export const MockEditor: React.FC<MockEditorProps> = ({ initialData, existingMocks = [], onSave, onDelete, onCancel, addToast }) => {
  ...
  const [fieldToDelete, setFieldToDelete] = useState<{ id: string, type: string, childrenCount: number } | null>(null);
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setFormData({ ...DEFAULT_MOCK, id: crypto.randomUUID(), createdAt: Date.now() });
      setIsRootArray(false);
      setVisualFields([
        { id: generateId(), key: 'id', value: '{{$uuid}}', type: 'string' },
        { id: generateId(), key: 'name', value: '{{$randomName}}', type: 'string' },
        { id: generateId(), key: 'metadata', value: '', type: 'object', children: [
            { id: generateId(), key: 'created', value: '{{$isoDate}}', type: 'string' }
        ]}
      ]);
    }
  }, [initialData]);
  ...
  useEffect(() => {
      if (formData.path && formData.method) {
          const isConflict = existingMocks.some(m => 
  ...
  useEffect(() => {
    if (editorMode === 'visual') {
      ...
      setFormData(prev => ({ ...prev, responseBody: JSON.stringify(finalJson, null, 2) }));
      setJsonError(null);
    }
  ...
  useEffect(() => {
    ...
    if (!showSearch) { setSearchTerm(''); setMatchCount({ current: 0, total: 0 }); }
  }, [showSearch]);
  ...
  const handleAddField = (parentId: string | null = null) => {
    ...
    const nextKey = (parentId === null && isRootArray) 
        ? String(visualFields.length) 
        : 'new_field';
  ...
  const executeRemoveField = (id: string) => {
    const removeRecursive = (list: SchemaField[]): SchemaField[] => {
      return list.filter(f => f.id !== id).map(field => {
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    if (e.key === 'Enter') {
        ...
        handleChange('responseBody', newValue);
        requestAnimationFrame(() => {
            if (textareaRef.current) textareaRef.current.selectionStart = textareaRef.current.selectionEnd = start + 1 + indent.length;
        });
        return;
    }
  };
    ...
    <div className="max-w-7xl mx-auto p-6 md:p-8 animate-enter h-full flex flex-col relative">
      <div className="flex items-center justify-between mb-6 flex-shrink-0">
        <div className="flex items-center space-x-4">
          <button type="button" onClick={onCancel} className="p-2.5 hover:bg-white hover:shadow-sm border border-transparent hover:border-slate-200 rounded-xl transition-all group">
            <ArrowLeft className="w-5 h-5 text-slate-500 group-hover:text-slate-800" />
          </button>
          <div>
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight">{initialData ? 'Edit Definition' : 'Design New Route'}</h1>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className={`flex items-center space-x-2 px-6 py-2.5 rounded-xl text-white font-bold text-sm transition-all shadow-lg active:scale-95 ${
              (jsonError || conflictError) ? 'bg-slate-400 cursor-not-allowed shadow-none' : 'bg-brand-600 hover:bg-brand-50 shadow-brand-500/25'
            }`}
          >
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-full overflow-hidden">
        ...
        <div className="lg:col-span-4 flex flex-col gap-6 overflow-y-auto pr-2 custom-scrollbar pb-10">
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                {conflictError ? (
                    ...
                    <div className="text-[10px] text-slate-400 mt-1.5 ml-1">Supports dynamic parameters (e.g. :id)</div>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                   ...
                   <div className="flex items-center justify-between bg-slate-50 px-3 py-2.5 rounded-xl border border-slate-200 h-[46px]">
                      <span className="text-xs font-semibold text-slate-600">{formData.isActive ? 'Active' : 'Disabled'}</span>
                      <button
                        type="button"
                        onClick={() => handleChange('isActive', !formData.isActive)}
                        className={`w-9 h-5 flex items-center rounded-full transition-colors duration-200 ease-in-out px-0.5 ${formData.isActive ? 'bg-green-500' : 'bg-slate-300'}`}
                      >
                        <div className={`w-4 h-4 bg-white rounded-full shadow-sm transform transition-transform duration-200 ${formData.isActive ? 'translate-x-4' : 'translate-x-0'}`} />
                      </button>
                  </div>
                </div>
              </div>
                    ...
                    {formData.authConfig?.type === 'API_KEY' && (
                         <div>
                            ...
                            <input
                              ...
                              onChange={(e) => setFormData(prev => ({
                                  ...prev,
                                  authConfig: { ...prev.authConfig!, headerKey: e.target.value }
                              }))}
                              className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2 text-xs font-mono focus:border-rose-500 outline-none"
                    ...
                    <div>
                        ...
                        <div className="relative">
                            ...
                            <input
                              ...
                              className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-9 pr-4 py-2 text-xs font-mono focus:border-rose-500 outline-none"
                              placeholder="secret-token-123"
                            />
                        </div>
                        <p className="text-[10px] text-slate-400 mt-1.5 ml-1">
                            Requests failing validation will return 401 Unauthorized.
                        </p>
                    </div>
                </div>
            )}
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4 relative overflow-hidden">
            ...
            <p className="text-xs text-slate-500 leading-relaxed">
                Connect this endpoint to an in-memory database collection to simulate real CRUD operations (Create, Read, Update, Delete).
            </p>
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-6">
            ...
            <div>
                <div className="flex justify-between items-center mb-4">
                  ...
                  <span className="text-xs font-mono font-bold text-brand-600 bg-brand-50 px-2.5 py-1 rounded-md border border-brand-100">{formData.delay}ms</span>
                </div>
                <input 
                  type="range" 
                  min="0" 
        ...
        <div className="lg:col-span-8 flex flex-col h-full bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden relative">
              ...
              <div className="flex-1 w-full h-full flex flex-col bg-[#1e1e2e] relative group">
                 <div className="flex-1 flex relative overflow-hidden">
                    ...
                    <div className="flex-1 relative w-full h-full overflow-hidden">
                        <div className="absolute inset-0 overflow-auto" style={{ scrollbarWidth: 'none' }}>
          ...
          <div className={`px-5 py-2.5 text-xs flex items-center border-t justify-between font-medium ${
            jsonError ? 'bg-red-50 text-red-600 border-red-200' : 'bg-slate-50 text-slate-500 border-slate-200'
          }`}>
            ...
            <span className="font-mono text-slate-400 flex items-center gap-1 opacity-70">
              <FileJson className="w-3.5 h-3.5"/> {editorMode === 'visual' ? 'Generated JSON' : 'Raw JSON'}
            </span>
          </div>
        </div>
      </form>
      ...
      {fieldToDelete && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
              <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6 animate-in zoom-in-95 duration-200 border border-slate-200" onClick={(e) => e.stopPropagation()}>
                  <div className="flex flex-col items-center text-center">
                      ...
                      <p className="text-sm text-slate-600 mb-6">
                          This field contains <strong className="text-slate-800">{fieldToDelete.childrenCount} nested items</strong>. Deleting it will permanently remove all child fields.
                      </p>
