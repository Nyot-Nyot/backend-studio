...
interface SidebarProps {
	...
	onPilihProyek: (id: string) => void;
	onBuatProyek: (name: string) => void;
	onHapusProyek: (id: string) => void;
...
export const Sidebar: React.FC<SidebarProps> = ({
	...
	const activeIndicator = (view: ViewState) =>
		currentView === view && (
			<div
				className={`absolute left-0 top-1/2 -translate-y-1/2 w-1 h-6 bg-brand-500 rounded-r-full shadow-[0_0_12px_rgba(14,165,233,0.6)] animate-in fade-in duration-300 ${
					isCollapsed ? "h-4" : "h-6"
				}`}
			/>
		);
	...
	return (
		...
		>
			...
			{/* Brand Header */}
			<div
				className={`p-6 pb-8 flex items-center ${isCollapsed ? "justify-center" : "space-x-3"} transition-all`}
			>
				...
				{!isCollapsed ? (
					<>
						<div className="flex items-center justify-between px-1 mb-2">
							...
							{proyek.length > 1 && (
								<button
									onClick={() => onHapusProyek(proyekAktifId)}
