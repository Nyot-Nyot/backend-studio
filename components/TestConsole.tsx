...
import { HttpMethod, MockEndpoint, TestConsoleState } from "../types";
			...
			<div className="bg-white border-b border-slate-200 px-8 py-8 shadow-sm z-10">
				<div className="max-w-6xl mx-auto">
					...
					<p className="text-slate-500 ml-[66px] text-sm max-w-2xl">
						Test your route designs instantly. Supports dynamic URL parameters (e.g., matching{" "}
						<code className="bg-slate-100 px-1 py-0.5 rounded text-slate-700">/users/:id</code>) and
						variable injection. Requests sent here appear in the Traffic Monitor.
					</p>
				</div>
			</div>
			...
			<div className="flex-1 p-8 max-w-6xl mx-auto w-full space-y-8 overflow-y-auto">
				...
				<div className="flex flex-col gap-4">
					<div className="bg-white p-2 rounded-2xl shadow-lg shadow-slate-200/50 border border-slate-200 flex flex-col md:flex-row gap-2">
						...
						<div className="flex gap-2">
							...
							<button
								...
								title={
									lastRequest ? `${lastRequest.method} ${lastRequest.path}` : "No previous request"
								}
							>
					...
					{(state.method === HttpMethod.POST ||
						state.method === HttpMethod.PUT ||
						state.method === HttpMethod.PATCH) && (
				...
				{state.response && (
					<div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
						...
						<div className="flex items-center justify-between">
							...
							<div className="text-xs text-slate-400">Size: {new Blob([state.response.body]).size} B</div>
						</div>
							...
							<div className="px-4 py-3 rounded-lg bg-red-50 border border-red-200 text-red-700 flex items-start gap-2">
								...
								<div>
									...
									<div className="text-xs opacity-80">
										Check your network connection or request body format.
									</div>
								</div>
							</div>
						)}
