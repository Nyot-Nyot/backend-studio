...
import { Database, Trash2, RefreshCw, Plus, Save, Search, Table } from 'lucide-react';
            ...
            <>
              <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                <div className="flex items-center gap-2">
                   ...
                   <span className="font-mono text-sm font-bold text-slate-700">{activeCollection}</span>
                   <span className="bg-slate-200 text-slate-600 text-[10px] px-2 py-0.5 rounded-full">{data.length} records</span>
                </div>
                <div className="flex items-center gap-2">
                  {isEditing ? (
                    <>
                        <button 
                            onClick={() => { setIsEditing(false); setRawEditor(JSON.stringify(data, null, 2)); setError(null); }}
                    ...
                    <div className="p-0 h-full overflow-y-auto">
                        {data.length === 0 ? (
                            <div className="h-full flex flex-col items-center justify-center text-slate-400">
                                ...
                                <p className="text-xs mt-1">POST requests to this endpoint will populate this list.</p>
                            </div>
                        ) : (
                                ...
                                <thead className="bg-slate-50 sticky top-0 shadow-sm">
                                    <tr>
                                        {Object.keys(data[0] || {}).slice(0, 5).map(key => (
                                            <th key={key} className="p-3 text-[10px] font-bold text-slate-500 uppercase tracking-wider border-b border-slate-200">{key}</th>
                                        ))}
                                    </tr>
                                </thead>
                                <tbody className="text-sm">
                                    {data.map((row, i) => (
                                        <tr key={i} className="border-b border-slate-100 hover:bg-slate-50/50 font-mono text-xs">
                                             {Object.keys(data[0] || {}).slice(0, 5).map(key => (
                                                <td key={key} className="p-3 text-slate-600 truncate max-w-[200px]">
