...
import { Trash2, Clock, Globe, BarChart2, Copy, Play, Pause, Link, Layers, GitBranch, Search, MoreHorizontal, ArrowRight, CheckSquare, X } from 'lucide-react';
...
export const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  const getMethodStyle = (method: HttpMethod) => {
    switch(method) {
      case HttpMethod.GET: return 'bg-blue-50 text-blue-700 border-blue-200 ring-blue-500/10';
      case HttpMethod.POST: return 'bg-emerald-50 text-emerald-700 border-emerald-200 ring-emerald-500/10';
      case HttpMethod.PUT: return 'bg-orange-50 text-orange-700 border-orange-200 ring-orange-500/10';
      case HttpMethod.DELETE: return 'bg-red-50 text-red-700 border-red-200 ring-red-500/10';
    ...
    <div className="p-8 max-w-[1600px] mx-auto space-y-10 animate-enter pb-20 relative">
      ...
      <div className="flex items-center justify-between sticky top-0 z-20 bg-[#f8fafc]/95 backdrop-blur-md py-4 border-b border-slate-200/50 -mx-8 px-8 transition-all">
        <div className="flex items-center space-x-2 text-sm text-slate-500">
           <span className="font-medium text-slate-700 bg-slate-200 px-2 py-0.5 rounded-full text-xs">{filteredMocks.length}</span> results found
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6">
        {filteredMocks.map((mock, index) => {
          ...
          return (
            ...
            >
              {/* Status Line */}
              <div className={`h-1 w-full transition-colors duration-300 ${mock.isActive ? 'bg-gradient-to-r from-brand-500 to-brand-400' : 'bg-slate-300'}`} />
              ...
              <div className="p-5 flex-1 flex flex-col relative">
                ...
                <div 
                  ...
                  onClick={(e) => handleSelect(e, mock.id)}
                >
                  <input 
                    type="checkbox" 
                    checked={isSelected}
                    readOnly
                    className="w-5 h-5 rounded border-gray-300 text-brand-600 focus:ring-brand-500 cursor-pointer accent-brand-600"
                  />
                </div>
                ...
                <div className="flex justify-between items-start mb-4 pl-6">
                  <div className="flex flex-col gap-1.5 min-w-0 pr-4">
                    ...
                    <div className="flex items-center space-x-2">
                      <span className={`px-2 py-0.5 rounded-md text-[10px] font-black uppercase tracking-wider border ring-1 ring-inset ${getMethodStyle(mock.method)}`}>
                        {mock.method}
                      </span>
                  ...
                  <div className="flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-all duration-200 translate-y-2 group-hover:translate-y-0 bg-white/90 backdrop-blur rounded-lg p-1 shadow-sm border border-slate-100 absolute top-4 right-4 z-10">
                    <ActionButton 
                      onClick={(e) => handleToggle(e, mock.id)} 
                      icon={mock.isActive ? <Pause className="w-3.5 h-3.5" /> : <Play className="w-3.5 h-3.5" />}
                      color={mock.isActive ? "text-amber-500 hover:bg-amber-50" : "text-emerald-500 hover:bg-emerald-50"}
        ...
        {filteredMocks.length === 0 && (
          <div className="col-span-full py-24 flex flex-col items-center justify-center text-center animate-in fade-in zoom-in-95 duration-300">
            ...
            <p className="text-slate-500 max-w-sm mx-auto mb-8">
              We couldn't find any endpoints matching "{searchTerm}". Try adjusting your search or create a new one.
            </p>
            <button 
          ...
          <div className="bg-slate-900 text-white rounded-full shadow-2xl shadow-slate-900/50 py-3 px-6 flex items-center gap-6 border border-slate-700">
            ...
            <div className="flex items-center gap-3">
              ...
              <button 
                ...
                className="text-sm text-red-400 hover:text-red-300 transition-colors flex items-center gap-2 px-2 py-1 rounded hover:bg-red-500/10"
              >
...
const StatCard = ({ icon, label, value, bg, border }: { icon: React.ReactNode, label: string, value: string | number, bg: string, border: string }) => (
  <div className={`p-6 rounded-2xl bg-white border shadow-sm flex items-center space-x-5 transition-transform hover:scale-[1.01] duration-300 ${border}`}>
...
const ActionButton = ({ onClick, icon, color, title }: { onClick: (e: React.MouseEvent) => void, icon: React.ReactNode, color: string, title: string }) => (
  <button
    onClick={onClick}
