...
// Alias ikon: gunakan nama-nama dalam bahasa Indonesia untuk konsistensi di file
const PanelKiriBuka = PanelLeftOpen;
const PanelKiriTutup = PanelLeftClose;
const Boks = Box;
const TongSampah2 = Trash2;
const ChevronTurun = ChevronDown;
const Cari = Search;
const Ceklis = Check;
const FolderTambah = FolderPlus;
const AlatTulis = PenTool;
const Percikan = Sparkles;
const DashboardLayout = LayoutDashboard;
const LabuPercobaan = FlaskConical;
const BasisData = Database;
...
export const Sidebar: React.FC<PropsSidebar> = ({
	...
	return (
		...
		>
			...
			>
				{sedangCollapsed ? <PanelKiriBuka className="w-3 h-3" /> : <PanelKiriTutup className="w-3 h-3" />}
			</button>
					...
					<>
						...
						<div className="flex items-center justify-between px-1 mb-2">
							...
							{daftarProyek.length > 1 && (
								...
								>
									<TongSampah2 className="w-3.5 h-3.5" />
								</button>
							)}
						</div>
						...
						<div className="relative group mb-3">
							...
							<div className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-slate-500 group-hover:text-slate-300 transition-colors">
								<ChevronTurun className="w-4 h-4" />
							</div>
						</div>
				...
				{sedangMembuat && !sedangCollapsed ? (
					<div className="bg-slate-800/50 p-3 rounded-xl border border-slate-700/50 animate-in fade-in slide-in-from-top-2 duration-200">
						...
						<div className="flex items-center space-x-2">
							...
							>
								<Ceklis className="w-3.5 h-3.5 mr-1.5" /> Buat
							</button>
					...
					!sedangCollapsed && (
						...
						>
							<FolderTambah className="w-3.5 h-3.5 transition-transform group-hover:scale-110" />
			...
			<div className={`px-4 space-y-8 flex-1 overflow-y-auto dark-scroll ${sedangCollapsed ? "px-2" : ""}`}>
				...
				<div className="space-y-3">
					...
					>
						<AlatTulis className="w-4 h-4 text-brand-400 group-hover:text-brand-300" />
					...
					{fitur.ai && (
						...
						>
							...
							<Percikan className="w-4 h-4 text-violet-200 group-hover:text-white transition-colors" />
							{!sedangCollapsed && (
