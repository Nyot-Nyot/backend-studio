...
**Contoh ringkas (konvensi nama + komentar):**

```ts
// Fungsi untuk menambahkan pengguna baru ke basis data.
// Mengembalikan objek pengguna yang baru dibuat.
export async function tambahPenggunaBaru(namaLengkap: string, email: string) {
	// Validasi sederhana: cek email
	if (!email.includes("@")) throw new Error("Email tidak valid");

	// Logika pembuatan pengguna dipisah ke fungsi terpisah agar lebih mudah diuji
	const penggunaBaru = await buatRekamPengguna({ nama: namaLengkap, email });
	return penggunaBaru;
}
```
